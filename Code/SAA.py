...
def sample_average_approximation( set_NO , params , earthquake_info = None):
    ...
    if m.status == gp.GRB.OPTIMAL:
        ...
        o_values = np.zeros((S,num_gathering_points))
        for s in range(S):
            for k in range(K_SD):
                g_SD_values[s,k] = g_SD[s,k].X
            for i in range(num_gathering_points):
                o_values[s,i] = o[s,i].X
    ...
    return m.ObjVal , final_time-current_time ,(u_values,v_values,w_values) , (g_SD_values , g_LD_values) , o_values